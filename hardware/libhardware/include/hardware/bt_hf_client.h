...
/**
 * Callback for sending incoming SCO over HCI data to app
 */
typedef void (* bthf_client_on_sco_data_callback) (const RawAddress *bd_addr,
                                                   const char* data,
                                                   size_t len);
...
typedef struct {
    ...
    bthf_client_on_sco_data_callback       sco_data_cb;
...
typedef struct {
    ...
    /** Send microphone data via SCO over HCI. */
    bt_status_t (*send_sco_ci_data) (const RawAddress *bd_addr, const char* data, size_t len);
