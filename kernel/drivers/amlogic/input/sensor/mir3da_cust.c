...
static const struct mir3da_sensor_name {
	const char *name;
	const char *description;
} mir3da_sensor_name[MIR3DA_SENSORS_NUMB] = {
	[MIR3DA_ACCEL] = {
		.name = "accelerometer",
		.description = "Mir3da Accelerometer Sensor",
	},
	[MIR3DA_SIGN_M] = {
			.name = "sign_m",
			.description = "Mir3da Significant Motion Sensor",
	},
	[MIR3DA_STEP_D] = {
			.name = "step_d",
			.description = "Mir3da Step Detector Sensor",
	},
	[MIR3DA_STEP_C] = {
			.name = "step_c",
			.description = "Mir3da Step Counter Sensor",
	},
	[MIR3DA_TILT] = {
			.name = "tilt",
			.description = "Mir3da Tilt Sensor",
	},
};

struct mir3da_sensor_data{
...
struct mir3da_data {
	...
	struct device *dev;
	struct mir3da_sensor_data sensors[MIR3DA_SENSORS_NUMB];
};
...
static void mir3da_work_func(struct work_struct *work)
{
  ...
  {	
    ...
    input_sync(mir3da->input); 
  	//printk("input data, x = %d, y = %d, z = %d",x, y, z);
  }
...
static const struct file_operations mir3da_misc_fops = {
        ...
        .compat_ioctl = mir3da_misc_ioctl,
};
...
static ssize_t mir3da_reset_step_counter(struct device *dev,
                    struct device_attribute *attr,
					const char *buf, size_t count){
	return count;
}

static ssize_t mir3da_step_count_show(struct device *dev,
		struct device_attribute *attr, char *buf){
	
	struct mir3da_sensor_data *sdata = dev_get_drvdata(dev);
	unsigned char step_temp[2];
	unsigned short count;

	mir3da_register_read_continuously(sdata->mir3da->mir3da_i2c_client, 0x0D, 2, step_temp);
...
static DEVICE_ATTR(poll_delay,      S_IRUGO |S_IWUSR|S_IWGRP,  mir3da_delay_show,              mir3da_delay_store);
...
static DEVICE_ATTR(step_count,	S_IRUGO,	mir3da_step_count_show,	NULL);
static DEVICE_ATTR(reset_steps,	S_IWUSR,	NULL,					mir3da_reset_step_counter);

#if 0	//TODO
static struct attribute *mir3da_accel_attribute[] = {
	&dev_attr_enable.attr,
...
static struct attribute *mir3da_sign_m_attribute[] = {
	&dev_attr_enable.attr,
	NULL,
};

static struct attribute *mir3da_step_d_attribute[] = {
	&dev_attr_enable.attr,
	NULL,
};

static struct attribute *mir3da_step_c_attribute[] = {
	&dev_attr_enable.attr,
...
static struct attribute *mir3da_tilt_attribute[] = {
	&dev_attr_enable.attr,
	NULL,
};

static const struct attribute_group mir3da_attribute_groups[] = {
	[MIR3DA_ACCEL] = {
		.attrs = mir3da_attributes,
		/* .attrs = mir3da_accel_attribute, */
		/* .name = "accel", */
	},
	[MIR3DA_SIGN_M] = {
		/* .name = "sign_m", */
		.attrs = mir3da_sign_m_attribute,
	},
	[MIR3DA_STEP_D] = {
		/* .name = "step_d", */
		.attrs = mir3da_step_d_attribute,
	},
	[MIR3DA_STEP_C] = {
		.attrs = mir3da_step_c_attribute,
		/* .name = "step_c", */
	},
	[MIR3DA_TILT] = {
		/* .name = "tilt", */
		.attrs = mir3da_tilt_attribute,
	},
};
...
static int  mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&mir3da->enable, 1);
    mir3da_set_enable(mir3da->mir3da_i2c_client, 1);
	schedule_delayed_work(&mir3da->work,msecs_to_jiffies(atomic_read(&mir3da->delay)));
...
static int i2c_static_add_device(struct i2c_board_info *info)
{
    ...
    client = i2c_new_device(adapter, info);
     MI_MSG(">>> mir3da_detect, client->addr = 0x%x\n",(unsigned int)info->addr) ;
