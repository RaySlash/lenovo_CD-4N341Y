...
#define MIR3DA_NSA_INIT_SECTION                         { NSA_REG_G_RANGE,              0x63,   0x61    },                                  \
                                                        { NSA_REG_POWERMODE_BW,         0xFF,   0x3c    },                                  \
                                                        { NSA_REG_ODR_AXIS_DISABLE,     0xFF,   0x08    },                                  \
...
int mir3da_chip_resume(MIR_HANDLE handle)
{
	...
	if(gsensor_chip_info.asic==ASIC_2513A){		
		...
		if(reg_data != 0xBB){
			MI_ERR("parse error chip");
			return -1;
		}

		mir3da_register_mask_write(handle, NSA_REG_INT_PIN_CONFIG, 0xff, 0x00);

		mir3da_register_read(handle, NSA_REG_CHIP_INFO, &reg_data);
		if(reg_data != 0x55){
