...
{
	unsigned int binv_cap, binv_play, finv, id;
	...
	pr_info("master_mode(%d), binv_p(%d), binv_c(%d), finv(%d) out_skew(%d), in_skew(%d)\n",
		master_mode, binv_play, binv_cap, finv, bclkout_skew, bclkin_skew);

	/* TDM out */
	if (playback_active) {
		aml_clk_set_tdmout_by_id(actrl,
			id, valb, valf,
			p_config->sclk_ws_inv, master_mode, binv_play);
