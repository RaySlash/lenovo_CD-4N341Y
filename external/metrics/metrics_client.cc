...
static int SendStats(char* argv[], int name_index, bool send_enum,
                     bool secs_to_msecs, bool send_to_autotest,
                     bool send_to_chrome) {
  ...
  if (send_to_chrome) {
    MetricsLibrary metrics_lib(nullptr);
    metrics_lib.Init();
    if (send_enum) {
...
int main(int argc, char** argv) {
  ...
  switch (mode) {
    case kModeSendStats:
      ...
      return SendStats(argv, arg_index, send_enum, secs_to_msecs,
                       send_to_autotest, send_to_chrome);
