...
class TaggedCounterInterface {
 public:
  ...
  typedef void (*Reporter)(void* handle, int32_t tag, int32_t count);
  ...
  virtual void Update(int32_t tag, int32_t count) = 0;
...
class TaggedCounter : public TaggedCounterInterface {
 public:
  ...
  virtual void Update(int32_t tag, int32_t count);
  virtual void Flush();
 ...
 private:
  ...
  class Record {
   public:
    ...
    // |count_| is set to 0.
    void Init(int32_t tag, int32_t count);
  ...
  // record.
  void UpdateInternal(int32_t tag, int32_t count, bool flush);
  ...
  // cached record.
  void ReportRecord(int32_t tag, bool flush);
  ...
  void UpdateRecord(int32_t tag, int32_t count, bool flush);
...
class TaggedCounterReporter : public TaggedCounterInterface {
 ...
 protected:
  ...
  static void Report(void* handle, int32_t tag, int32_t count);

  static MetricsLibraryInterface* metrics_lib_;
  std::unique_ptr<TaggedCounter> tagged_counter_;
...
class FrequencyCounter {
 public:
  ...
  // time of the call.
  virtual void Update(int32_t count) {
 ...
 private:
  ...
  void UpdateInternal(int32_t count, time_t now);
  int32_t GetCycleNumber(time_t now);

  time_t cycle_duration_;
  std::unique_ptr<TaggedCounterInterface> tagged_counter_;
};
