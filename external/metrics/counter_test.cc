...
static const int64_t kint32max = std::numeric_limits<int32_t>::max();
...
class TaggedCounterTest : public testing::Test {
 protected:
  ...
  virtual void TearDown() {
    ...
    test_ = NULL;
    base::DeleteFile(base::FilePath(kTestRecordFile), false);
  }
  ...
  void ExpectReporterCall(int32_t tag, int32_t count) {
  ...
  static void Reporter(void* handle, int32_t tag, int32_t count) {
  ...
  StrictMock<MockFunction<void(void* handle,
                               int32_t tag, int32_t count)> > reporter_;
...
TEST_F(TaggedCounterTest, BadFileLocation) {
  ...
  EXPECT_EQ(TaggedCounter::kRecordInvalid, counter_.record_state_);
  base::DeleteFile(base::FilePath(kDoesNotExistFile), false);
}
...
class FrequencyCounterTest : public testing::Test {
 protected:
  ...
  void CheckInit(int32_t cycle_duration);
  void CheckCycleNumber(int32_t cycle_duration);
...
void FrequencyCounterTest::CheckInit(int32_t cycle_duration) {
  tagged_counter_ = new StrictMock<TaggedCounterMock>;
...
void FrequencyCounterTest::CheckCycleNumber(int32_t cycle_duration) {
