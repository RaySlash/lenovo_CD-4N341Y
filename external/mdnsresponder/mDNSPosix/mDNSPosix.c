...
#elif defined(BUILD_EUREKA)
	strncpy(&namelabel->c[1], "Chromecast", MAX_DOMAIN_LABEL);
#else
...
// be torn down and rebuilt, return affected indices as a bitmask. Otherwise return 0.
	...
	while (1)
		...
		// For Eureka, it's possible to get NEWLINK message without real change in interface.
		if (pNLMsg->nlmsg_type == RTM_GETLINK || (pNLMsg->nlmsg_type == RTM_NEWLINK && ((struct ifinfomsg*) NLMSG_DATA(pNLMsg))->ifi_change))
#else
