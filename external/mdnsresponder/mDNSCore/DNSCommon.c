...
#if defined(BUILD_EUREKA)
// Log less freqntly than every 30 seconds.
void logSendDNSMessage(mDNS *const m)
	{
	static int mdns_response_sent = 0;
	static int last_time_logged = 0;
	...
	++mdns_response_sent;
	if (last_time_logged == 0)
		{
		last_time_logged = info.uptime;
		}
	else if (info.uptime > (last_time_logged + logging_interval))
		{
		LogMsg("mDNS Response sent for %d seconds = %d\n", info.uptime - last_time_logged, mdns_response_sent);
		mdns_response_sent = 0;
		last_time_logged = info.uptime;
		}
