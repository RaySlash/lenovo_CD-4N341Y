			  ...
			  int only_first_ssid)
{
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		struct wpa_ssid *ssid_match = wpa_scan_res_match(wpa_s, i, bss, group,
				only_first_ssid);
		...
		if (!result) {
			*selected_ssid = ssid_match;
			result = bss;

			if (wpa_s->wpa_state < WPA_ASSOCIATED) {
				wpa_dbg(wpa_s, MSG_INFO, "Selected BSS :%02x"
					" (level:%d noise:%d frequency:%d)",
					bss->bssid[5],
					bss->level, bss->noise, bss->freq);
			}
		} else {
			if (wpa_s->wpa_state < WPA_ASSOCIATED) {
				wpa_dbg(wpa_s, MSG_INFO, "Other matching BSS :%02x"
					" (level:%d noise:%d frequency:%d)",
					bss->bssid[5],
					bss->level, bss->noise, bss->freq);
			}
