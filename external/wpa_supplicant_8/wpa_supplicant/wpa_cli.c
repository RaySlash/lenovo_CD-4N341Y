...
static int wpa_cli_open_connection(const char *ifname, int attach)
{
	...
	/* for Eureka, ignore ctrl_iface_dir, always use Android socket at /dev/socket/ */
	cfile = os_strdup(ifname);
	if (cfile == NULL)
		return -1;
#endif /* ANDROID, BUILD_EUREKA */
...
static int _wpa_ctrl_command(struct wpa_ctrl *ctrl, char *cmd, int print)
{
	...
	// In MTV-45, dual-band device may scan up to 200 APs. b/21699116.
	char buf[16 * 1024];
#else
	char buf[4096];
#endif
