...
#if defined(BUILD_EUREKA) && defined(CUSTOM_WIFI_SNR)
#include "custom_wifi_snr.h"
#endif  // BUILD_EUREKA && CUSTOM_WIFI_SNR
 ...
 * recommends 25 as a minimum SNR for 54 Mbps data rate.
 * Originally, 30 is chosen here as a conservative value.
 * For Earth/Hendrix, we override this to 25dB:
 *  - HDMI interferes on 2.4GHz.
 *  - We can be less conservative because we aren't a mobile device.
 *  - We don't want a fade to disqualify a desired 5GHz network.
 */
#define GREAT_SNR 25
 ...
 */
#define DEFAULT_NOISE_FLOOR_2GHZ (-80)
#define DEFAULT_NOISE_FLOOR_5GHZ (-88)

static bool is_5ghz(int frequency)
{
	return frequency > 4000;
}

#ifdef CUSTOM_WIFI_SNR
static int get_snr(int sig, int noise, int freq) {
	return get_custom_snr(sig, noise, freq);
}
#else
// This must be kept synchronized with the function of the same name in net_mgr.
// Otherwise there may be loops of disconnects. See b/24093114.
static int get_snr(int signal_level, int noise, int frequency) {
	// Threshold for "Good SNR".
	static const double kGoodSnr = 20.0;

	// Default noise level.
	static const double kDefaultNoise = -80.0;

	// Noise advantage for 5Ghz.
	static const double kNoiseBoost5ghz = 8.0;

	// Penalty multiplier for 2.4GHz SNR > kGoodSnr.
	static const double kPenaltyDbFactor2Ghz = 3.0;

	double snr = signal_level - kDefaultNoise;

	if (is_5ghz(frequency)) {
		snr += kNoiseBoost5ghz;
	} else {
		if (snr > kGoodSnr)
			snr = kGoodSnr + (snr - kGoodSnr) / kPenaltyDbFactor2Ghz;
	}

	return (int)snr;
}
#endif  // CUSTOM_WIFI_SNR
...
static int wpa_scan_result_compar(const void *a, const void *b)
{
	...
	if (!(wa->flags & wb->flags & WPA_SCAN_LEVEL_DBM)) {
		...
		snr_a = get_snr(wa->level, wa->noise, wa->freq);
		snr_b = get_snr(wb->level, wb->noise, wb->freq);
	}
