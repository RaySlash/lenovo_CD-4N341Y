				...
				const char *value)
{
	...
	if (hexstr2bin(value, ssid->psk, PMK_LEN) ||
	    value[PMK_LEN * 2] != '\0') {
#ifdef EUREKA
		wpa_printf(MSG_ERROR, "Line %d: Invalid PSK.", line);
#else
				    ...
				    const char *value, int idx)
{
	...
	if (buf == NULL) {
#ifdef EUREKA
		wpa_printf(MSG_ERROR, "Line %d: Invalid WEP key %d.",
			   line, idx);
#else
...
#ifdef EUREKA
		wpa_printf(MSG_ERROR, "Line %d: Too long WEP key %d.",
			   line, idx);
#else
...
static char * wpa_config_write_wep_key(struct wpa_ssid *ssid, int idx)
{
	if (ssid->wep_key_len[idx] == 0)
		return NULL;
	return wpa_config_write_string_hex(ssid->wep_key[idx],
...
#ifdef WPA_UNICODE_SSID
	{ STR_RANGE_UNICODE(ssid, 0, SSID_MAX_LEN) },
#else
