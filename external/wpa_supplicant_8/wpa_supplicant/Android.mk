...
include $(LOCAL_PATH)/eureka.config
...
# Add EUREKA definition.
L_CFLAGS += -DEUREKA
# To allow non-ASCII characters in SSID
L_CFLAGS += -DWPA_UNICODE_SSID

ifeq ($(BUILD_EUREKA),true)
# Use eureka specific directory for control interface sockets
L_CFLAGS += -DCONFIG_CTRL_IFACE_CLIENT_DIR=\"/dev/socket/wifi\"
L_CFLAGS += -DCONFIG_CTRL_IFACE_DIR=\"/data/wifi\"
# # Use eureka specific directory for wpa_cli command completion history
L_CFLAGS += -DCONFIG_WPA_CLI_HISTORY_DIR=\"/data/wifi\"
else
...
LOCAL_CFLAGS := -UANDROID $(L_CFLAGS)
...
ifeq ($(BUILD_EUREKA),true)
ifeq ($(CUSTOM_WIFI_SNR),true)
LOCAL_CFLAGS += -DCUSTOM_WIFI_SNR
LOCAL_STATIC_LIBRARIES += custom_wifi_snr
...
LOCAL_TOOLCHAIN_PREBUILTS := \
  $(LOCAL_PATH)/src/common/wpa_ctrl.h:usr/include/wpa_ctrl.h
