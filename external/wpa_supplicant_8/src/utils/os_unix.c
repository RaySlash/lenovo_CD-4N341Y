...
int os_program_init(void)
{
	...
	int capabilities[] = {CAP_NET_ADMIN, CAP_NET_RAW};
	int i;
	for (i = 0; i < sizeof(capabilities)/sizeof(capabilities[0]); ++i) {
		int idx = CAP_TO_INDEX(capabilities[i]);
		cap[idx].effective |= CAP_TO_MASK(capabilities[i]);
		cap[idx].permitted |= CAP_TO_MASK(capabilities[i]);
		cap[idx].inheritable = 0;
	}

	capset(&header, cap);
#endif /* ANDROID || BUILD_EUREKA */
