...
#define dump_attr(name) do { \
	if (attr.name) { \
		char *str = os_malloc(attr.name##_len + 1); \
		size_t i; \
		if (str == NULL) \
			return pos - buf; \
		for (i = 0; i < attr.name##_len; i++) { \
			if (attr.name[i] < 32) \
				str[i] = '_'; \
			else \
				str[i] = attr.name[i]; \
		...
		str[i] = '\0'; \
		ret = os_snprintf(pos, end - pos, "wps_%s=%s\n", #name, str); \
		os_free(str); \
	...
	dump_attr(manufacturer);
	dump_attr(model_name);
	dump_attr(model_number);
	dump_attr(serial_number);
	dump_attr(dev_name);
