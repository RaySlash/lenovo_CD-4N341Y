...
"MHcCAQEEIEXaoUzRpbhFqkitaBaLZvdPQujpt9CRpBOyiy9mcP5noAoGCCqGSM49\n"
"AwEHoUQDQgAErz/GZj6AjoJY6+nUC0+yJ6FA9OoZk2h0bFhh7GaGJz4c1o6ZorEV\n"
"RmPmZC1rXgJvlz9zdzIw5I7pLoFGwgm2NA==\n"
"-----END EC PRIVATE KEY-----";
...
int main(void)
{
	...
	char prikey[512];
	char* did;
	did = (unsigned char*)malloc(sizeof(unsigned char)*1024);
	...
	smac = (unsigned char*)malloc(sizeof(unsigned char)*512);
		...
		for (int k = 0; k< strlen((unsigned char *)mac_decrypt); k++) {
        	sprintf(smac+k*2,"%02x", mac_decrypt[k]);
    	}
		smac[strlen(smac)] = '\0';
	...
	ret = system(" openssl ecparam -name prime256v1 -genkey -noout -out /data/private.pem && openssl ec -in /data/private.pem -pubout -out /data/public.pem ");
...
#endif	
	sprintf(did,"%s.%s.null",ssn,smac);
	did[strlen(ssn)+strlen(smac)+strlen("null")+2*strlen(".")] = '\0';
	printf(" did:\n%s\n",did);
