...
void reboot_service(int fd, void *arg)
{
    ...
    // Eureka: Close the connection immediately, to prevent the client from
    // hanging.
    adb_close(fd);
    sync();
...
#ifdef ADB_SECURE
        // This variable can be used to trigger secure ADB
        // functionality in the executable.
        setenv("ADB_SECURE", "1", 1);
#endif  // ADB_SECURE
...
static int create_subproc_thread(const char *name)
{
    ...
    if (name) {
#ifdef ADB_SECURE
        if (IsSafeCommand(name)) {
            char* first_space = strchr(name, ' ');
            if (first_space == NULL) {
                // No arguments, so just run the executable
                ret_fd = create_subprocess(name, (char*)NULL, (char*)NULL,
                                           &pid);
            } else {
                char* executable_path = strndup(name, first_space - name);

                // Pass the list of arguments as a single argument to the
                // executable. The args will have to be split up later if
                // desired, which is easily achieved in a shell script with
                // `set -- $@`. This avoids having to parse arguments here.
                ret_fd = create_subprocess(executable_path, first_space + 1,
                                           (char*)NULL, &pid);
                free(executable_path);
            }
        } else {
            ret_fd = create_subprocess(SHELL_COMMAND, "-c",
                                       "echo 'error: illegal command'", &pid);
        }
#else
...
#ifdef ADB_SECURE
        // Don't allow interactive shell.
        ret_fd = create_subprocess(SHELL_COMMAND, "-c",
                                   "echo 'error: interactive shell disabled'",
                                   &pid);
#else
...
int service_to_fd(const char *name)
{
...
#ifdef ADB_SECURE
    // Only allow shell and sync operations. Their functionality is further
    // restricted in their respective implementations.
    if (strncmp(name, "shell:", 6) && strncmp(name, "sync:", 5)) {
        return ret;
    }
#endif  // ADB_SECURE
    ...
    if(!strncmp(name, "tcp:", 4)) {
        ...
        name = strchr(name + 4, ':');
        if(name == 0) {
            ret = socket_network_client("127.0.0.1", port, SOCK_STREAM);
            if (ret >= 0)
