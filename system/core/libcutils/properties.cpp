...
// Convert string property to int (default if fails); return default value if
// out of bounds
static intmax_t property_get_imax(const char* key,
  ...
  if (len > 0) {
    ...
    if ((result == INTMAX_MIN || result == INTMAX_MAX) && errno == ERANGE) {
      ...
      result = default_value;
      ALOGV("%s(%s,%" PRIdMAX ") - numeric conversion failed", __FUNCTION__,
            key, default_value);
    }
...
int property_list(void (*fn)(const char* name, const char* value, void* cookie),
                  void* cookie) {
