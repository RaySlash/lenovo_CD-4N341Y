  ...
  if (bufID != LOG_ID_RADIO) {
    switch (tag[0]) {
      ...
      case 'R':
        ...
        if (strncmp(tag + 1, &"RIL"[1], sizeof("RIL") - 2)) break;
        goto inform;
      case 'Q':
        /* Any log tag with "QC_RIL" as the prefix */
        if (strncmp(tag + 1, &"QC_RIL"[1], sizeof("QC_RIL") - 2)) break;
        goto inform;
      case 'I':
        /* Any log tag with "IMS" as the prefix */
        if (strncmp(tag + 1, &"IMS"[1], sizeof("IMS") - 2)) break;
        goto inform;
      case 'A':
...
LIBLOG_ABI_PUBLIC int __android_log_buf_vprint(int bufID, int prio, const char *tag, const char *fmt, va_list ap) {
  char buf[LOG_BUF_SIZE];
  vsnprintf(buf, LOG_BUF_SIZE, fmt, ap);
  return __android_log_buf_write(bufID, prio, tag, buf);
}
...
LIBLOG_ABI_PUBLIC int __android_log_clear(int bufID) {
  ssize_t ret = -EINVAL;

  struct logger_list* logger_list;
  struct logger* logger;

  logger_list = android_logger_list_alloc(ANDROID_LOG_WRONLY, 0, 0);
  if (logger_list == NULL) {
    return ret;
  }

  logger = android_logger_open(logger_list, (log_id_t)bufID);
  if (logger == NULL) {
    return ret;
  }

  ret = android_logger_clear(logger);
