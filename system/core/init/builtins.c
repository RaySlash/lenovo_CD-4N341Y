...
extern void add_environment(const char *name, const char *value);
...
#if HAVE_GLIBC
/* setkey conflicts with an already defined function */
#define setkey setkey_kbe
#endif
...
static int run_application(const char *path, char **args)
{
    sigset_t set, oldset;
    int err, status;
    ...
    if (pid == 0) {
        ...
        /* Close all parent file handles except property workspace */

        /* Create new session */
        setsid();

        /* We don't want to use real console here - it will become controlling
         * terminal, just pick something usable for debugging output.
         */
    ...
    if (WIFEXITED(status)) {
        ...
        err = WTERMSIG(status);
        ERROR("'%s' was terminated by signal %d\n", path, err);
    } else {
        ERROR("'%s' exited with weird status 0x%08x\n", path, status);
        err = -1;
    }
...
{
    int res;
    char **argv = malloc(sizeof(char*) * nargs);
    if (argv == NULL) {
        ERROR("out of memory");
        return -1;
    }
    memcpy(argv, args + 1, sizeof(char*) * (nargs - 1));
    argv[nargs - 1] = NULL;
    res = run_application(args[1], argv);
    free(argv);
    return res;
}
...
int do_mount(int nargs, char **args)
{
    ...
    for (n = 4; n < nargs; n++) {
        ...
        if (!mount_flags[i].name) {
            if (!strcmp(args[n], "wait"))
                wait = 1;
            else if (!strcmp(args[n], "slotselect"))
                slotselect = 1;
    ...
    target = args[3];
    if (slotselect) {
      /* Append ro.boot.slot_suffix to source */
      const char *slot_suffix = property_get("ro.boot.slot_suffix");
      snprintf(source_slot, sizeof(source_slot), "%s%s", source, slot_suffix);
      source = source_slot;
    }
