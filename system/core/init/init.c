...
// Define these if they are missing from <linux/capability.h>
...
/* Need to delay checking to console in order to support USB */
...
int add_environment(const char *key, const char *val)
{
    ...
    ERROR("Maximal number of environments reached.\n");
    return 1;
}
...
static void open_console()
{
    ...
    const char *usb_console = "/dev/ttyUSB0";
    char *new_console = usb_console;
    while (stat(usb_console, &console_stat) == -1) {
        struct sysinfo s_info;
        if (sysinfo(&s_info) == -1 || s_info.uptime > 15) {
            /* Assume no USB tty is available, use backup */
            new_console = console_name;
            break;
        }
        /* We may need to wait for USB tty device to be created */
        sleep (1);
    }

    int fd;
    if ((fd = open(new_console, O_RDWR)) < 0) {
        fd = open("/dev/null", O_RDWR);
    }
...
void service_start(struct service *svc, const char *dynamic_args)
{
...
#endif
    const char *pval;
    pval = property_get("ro.secure");
    if (pval) {
        if (!strcmp(pval, "1")) {
            if (!strcmp(svc->name, "sshd") ||
                   !strcmp(svc->name, "adbd") ||
                   !strcmp(svc->name, "console")) {
                INFO("don't start %s services\n", svc->name);
                return;
            }
        }
    }
    ...
    if (pid == 0) {
        ...
        struct __user_cap_data_struct caps[_LINUX_CAPABILITY_U32S_3] = { 0 };
        ...
        {
            char prefix[128];
            snprintf(prefix, sizeof(prefix), "svc.%s.", svc->name);
            add_environment("ANDROID_WAKELOCK_PREFIX", prefix);
        }
#endif
...
#endif
        if (svc->writepid_files) {
            char pid_str[16];
            int pid_str_len = snprintf(pid_str, sizeof(pid_str), "%d", getpid());
            if (pid_str_len > 0 && pid_str_len < sizeof(pid_str)) {
                struct stringlist* file = svc->writepid_files;
                 while (file) {
                      if (!write_string_to_file(file->value, pid_str, pid_str_len)) {
                          ERROR("couldn't write %s to %s: %s\n",
                                pid_str, file->value, strerror(errno));
                      }
                      file = file->next;
                 }
            } else {
              ERROR("Failed to write pid for writepid option.\n");
            }
        }
        ...
        if (svc->capabilities != 0 && prctl(PR_SET_KEEPCAPS, 1, 0, 0, 0) != 0) {
            ERROR("prtcl(PR_SET_KEEPCAPS) failed: %s\n", strerror(errno));
            _exit(127);
        }
    ...
    /* as requested, set our gid, supplemental gids, and uid */
        ...
        if (pw && pw->pw_dir) {
            add_environment("HOME", pw->pw_dir);
        }
        ...
        if (svc->capabilities != 0) {
            ...
            // Add the capabilities to the ambient set so they are preserved
            // across execve
            // (http://man7.org/linux/man-pages/man7/capabilities.7.html)
            for (size_t cap = 0; cap <= CAP_LAST_CAP; ++cap) {
                if (svc->capabilities & (1LL << cap)) {
                    if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, cap, 0, 0)) {
                        ERROR("prctl(PR_CAP_AMBIENT) failed: %s\n", strerror(errno));
                        _exit(127);
                    }
                }
            }
...
static void service_stop_or_reset(struct service *svc, int how)
{
    ...
    if (svc->pid) {
        ERROR("service '%s' pid %d is being killed\n", svc->name, svc->pid);
...
static void import_kernel_nv(char *name, int for_emulator)
{
    ...
    *value++ = 0;
    ...
    /* if cpu_rev is given on kernel command line, override what we read before
     * (e.g. from /proc/cpuinfo), if anything */
    if (!strcmp(name, "cpu_rev")) {
...
static void export_kernel_boot_props(void)
{
    ...
    struct {
        ...
        { "ro.boot.debuggable", "ro.debuggable", "1", },
        { "ro.boot.secure", "ro.secure", "0", },
        { "ro.boot.lang", "ro.lang", "en", },
        { "ro.boot.country", "ro.country", "US", },
        { "ro.boot.locale.lang", "ro.product.locale.language", "en", },
        { "ro.boot.locale.region", "ro.product.locale.region", "US", },

    };

    for (i = 0; i < ARRAY_SIZE(prop_map); i++) {
        pval = property_get(prop_map[i].src_prop);
        if (!strcmp(prop_map[i].dest_prop, "ro.country") && pval && !strncmp(pval, "WW", 2)) {
            property_set(prop_map[i].dest_prop, "WW");
        } else {
...
#ifdef BUILD_EUREKA
    // Modern Android doesn't integrate the policy version in the file name.
    strncpy(path, path_prefix, sizeof(path));
