...
// TODO(bcf): Audit this list, remove unneeded/dangerous capabilities
...
static int lookup_cap(const char *name) {
    for (size_t i = 0; i < ARRAY_SIZE(cap_map); ++i) {
        if (!strcmp(name, cap_map[i].name))
...
int lookup_keyword(const char *s)
{
    ...
    if (!strcmp(s, "opy")) return K_copy;
        if (!strcmp(s, "apabilities")) return K_capabilities;
...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    case K_capabilities: {
        for (int i = 1; i < nargs; ++i) {
            int cap = lookup_cap(args[i]);
            if (cap == -1) {
                parse_error(state, "Invalid capability %s\n", args[i]);
            } else {
                assert(cap_valid(cap));
                svc->capabilities |= (1LL << cap);
            }
        }
        break;
    }
    ...
    case K_writepid: {
        ...
        for (i = 1; i < nargs; i++) {
            ...
            tmp->value = strdup(args[i]);
            tmp->next = svc->writepid_files;
            svc->writepid_files = tmp;
        }
        break;
    }
    case K_user:
