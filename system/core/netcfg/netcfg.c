...
static int set_hwaddr(const char *name, const char *asc) {
...
static int set_hwprefix(const char *name, const char *asc_prefix) {
    struct ether_addr addr;
    if (ifc_get_hwaddr(name, &addr.ether_addr_octet) < 0) {
        printf("Failed to get current hw address of '%s'\n", name);
        return -1;
    } else {
        unsigned char asc[ETH_ALEN * 3];
        unsigned char* octet = addr.ether_addr_octet;
        sprintf(asc, "%02x:%02x:%02x:%02x:%02x:%02x",
                octet[0], octet[1], octet[2], octet[3], octet[4], octet[5]);
        int prefix_len = strlen(asc_prefix);
        if (prefix_len >= sizeof(asc)) {
            prefix_len = sizeof(asc) - 1;
        }
        memcpy(asc, asc_prefix, prefix_len);
        return set_hwaddr(name, asc);
    }
}
